...
def detect_events_two_stage(feature_traces, feature_times, trial_rwd_times,
                             primary_feature='solenoid_derivative',
                             lfp_prefix='lfp_block_',
                             primary_window=(-0.1, 0.5),
                             lfp_refine_window=0.15,
                             min_confidence=0.3,
                             verbose=True):
    """
    Two-stage alignment:
      Stage 1 — solenoid_derivative anchors each trial (100% hit rate).
      Stage 2 — best LFP block refines timing in a narrow ±150ms window
                around the solenoid candidate (when a credible peak exists).

    Returns
    -------
    events        : list of per-trial dicts (same format as detect_ephys_events)
    event_sources : list of strings — 'solenoid', 'solenoid+lfp', 'no_detection'
    """
    # ── Stage 1 fallback if solenoid_derivative is missing ───────────────
    if primary_feature not in feature_traces:
        for alt in ['solenoid_global', 'solenoid_block']:
            if alt in feature_traces:
                primary_feature = alt
                break

    if primary_feature not in feature_traces:
        if verbose:
            print("[FUSION] No solenoid feature found — falling back to equal-weight fusion")
        events, _ = detect_ephys_events(
            feature_traces, feature_times, trial_rwd_times,
            search_window=primary_window,
            min_confidence=min_confidence)
        return events, ['equal_weight'] * len(trial_rwd_times)

    sol_trace = feature_traces[primary_feature]
    sol_times = feature_times[primary_feature]
    lfp_features = {k: v for k, v in feature_traces.items()
                    if k.startswith(lfp_prefix)}

    n_trials = len(trial_rwd_times)
    events        = [None] * n_trials
    event_sources = ['no_detection'] * n_trials

    for tt in range(n_trials):
        t_rwd   = float(trial_rwd_times[tt])
        t_start = t_rwd + primary_window[0]
        t_end   = t_rwd + primary_window[1]

        # ── Stage 1: solenoid candidate ──────────────────────────────────
        sol_mask = (sol_times >= t_start) & (sol_times <= t_end)
        if sol_mask.sum() < 1:
            continue

        sol_t_win  = sol_times[sol_mask]
        sol_tr_win = sol_trace[sol_mask]
        best_sol_idx = int(np.argmax(sol_tr_win))
        t_sol  = float(sol_t_win[best_sol_idx])
        z_sol  = float(sol_tr_win[best_sol_idx])

        if z_sol < 1.0:   # should never happen for solenoid, but guard anyway
            continue

        # ── Stage 2: LFP refinement in narrow window around solenoid ─────
        best_lfp_t = None
        best_lfp_z = 0.0

        for lfp_name, lfp_trace in lfp_features.items():
            lfp_t_arr = feature_times[lfp_name]
            lw0 = t_sol - lfp_refine_window
            lw1 = t_sol + lfp_refine_window
            lfp_mask = (lfp_t_arr >= lw0) & (lfp_t_arr <= lw1)
            if lfp_mask.sum() < 2:
                continue

            lfp_t_win  = lfp_t_arr[lfp_mask]
            lfp_tr_win = lfp_trace[lfp_mask]
            pk_idx = int(np.argmax(lfp_tr_win))
            lfp_z  = float(lfp_tr_win[pk_idx])

            if lfp_z > best_lfp_z and lfp_z > 0.8:
                best_lfp_z = lfp_z
                best_lfp_t = float(lfp_t_win[pk_idx])

        # ── Final time: weighted centroid, LFP capped at 50% of solenoid ─
        if best_lfp_t is not None:
            w_sol  = z_sol
            w_lfp  = min(best_lfp_z, z_sol * 0.5)   # cap prevents LFP dominating
            t_final = (w_sol * t_sol + w_lfp * best_lfp_t) / (w_sol + w_lfp)
            conf   = min(1.0, (z_sol + best_lfp_z) / (z_sol + best_lfp_z + 2.0))
            source = 'solenoid+lfp'
        else:
            t_final = t_sol
            conf    = min(1.0, z_sol / (z_sol + 3.0))
            source  = 'solenoid'
        ...
        events[tt] = {
            ...
            'confidence':     conf,
            'n_candidates':   1,
            'feature_scores': {primary_feature: z_sol, 'lfp_best': best_lfp_z},
            'method':         source,
        }
        event_sources[tt] = source

    if verbose:
        n_s   = sum(1 for s in event_sources if s == 'solenoid')
        n_sl  = sum(1 for s in event_sources if s == 'solenoid+lfp')
        n_nd  = sum(1 for s in event_sources if s == 'no_detection')
        print(f"[FUSION] solenoid-only: {n_s}  solenoid+lfp: {n_sl}  "
              f"no_detection: {n_nd}  (of {n_trials} trials)")
...
def process_one_session(bhv_file, ap_bin=None, lf_bin=None, ks_folder=None,
    ...
    # ── Detect events (two-stage: solenoid anchor + LFP refinement) ──────
    events, event_sources = detect_events_two_stage(
        feature_traces, feature_times, trial_rwd_times,
        primary_feature='solenoid_derivative',
        lfp_prefix='lfp_block_',
        primary_window=(-0.1, 0.5),
